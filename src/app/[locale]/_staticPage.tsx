...
type StaticPageConfig = {
  /** Contentful entry slug — shared across all locales */
  slug: string
  /** Locale-keyed URL segments for canonical/alternates */
  segments: Record<Locale, string>
}

export function makeStaticPageMetadata({ slug, segments }: StaticPageConfig) {
  return async function generateStaticPageMetadata({ params }: Props): Promise<Metadata> {
    const { locale } = await params
    const { isEnabled: preview } = await draftMode()
    const page = await getStaticPageBySlug(slug, locale, preview)
    ...
    return {
      ...
      alternates: {
        canonical: `${SITE_URL}/${locale}/${segments[locale]}`,
        languages: {
          en: `${SITE_URL}/en/${segments.en}`,
          fr: `${SITE_URL}/fr/${segments.fr}`,
          'x-default': `${SITE_URL}/en/${segments.en}`,
        },
      },
      openGraph: {
        type: 'website',
        ...
        url: `${SITE_URL}/${locale}/${segments[locale]}`,
        title: m.title ?? undefined,
        description: m.description ?? undefined,
...
export function makeStaticPageRoute({ slug }: Pick<StaticPageConfig, 'slug'>) {
  return async function StaticPageRoute({ params }: Props) {
    const { locale } = await params
    const { isEnabled: preview } = await draftMode()
    const [page, dict] = await Promise.all([
      getStaticPageBySlug(slug, locale, preview),
