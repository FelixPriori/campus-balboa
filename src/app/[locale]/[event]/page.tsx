			...
			<main className={styles.listingPage}>
					...
					<section className={styles.section} aria-labelledby="upcoming-heading">
						...
						<ul className={styles.eventGrid}>
							{upcomingEvents.map((ev, i) => (
								<EventCard key={ev.slug ?? i} ev={ev} index={i} locale={locale} eventSegment={eventSegment} />
							))}
						</ul>
					</section>
				)}
				...
				{pastEvents.length > 0 && (
					<section className={styles.section} aria-labelledby="past-heading">
						...
						<ul className={styles.eventGrid}>
							{pastEvents.map((ev, i) => (
								<EventCard key={ev.slug ?? i} ev={ev} index={i} locale={locale} eventSegment={eventSegment} past />
							))}
						</ul>
					</section>
				)}
