...
function BlogSpot({ section, subSection, item }: { section: string, subSection: string, item: string }) {
    const tItem = useTranslations(`Events.2024.MtlBalJam.visitingPage.travelBlogSection.${section}.${subSection}.${item}`)
    const tLegend = useTranslations('Events.2024.MtlBalJam.visitingPage.travelBlogSection.legend');
    ...
    return (
        <p className={styles.blogSpot}>
            <span className={styles.spotName}>{tItem(`name`)}</span><span> - </span>
            <span className={styles.spotType}>{tItem(`type`)}</span><span> | </span>
            {price && <><span className={styles.spotPrice}>{price}</span><span> | </span></>}
            {spec && <><span className={styles.spotSpec}><abbr title={`${tLegend('veg')}\n${tLegend('gluten')} `}>{spec}</abbr></span><span> | </span></>}
            <span className={styles.spotNeighborhood}>{tItem(`neighborhood`)}</span>
        </p>
    )
}

function BlogSubSection({ section, subSection, subSectionItems }: { section: string, subSection: string, subSectionItems: string[] }) {
    const t = useTranslations(`Events.2024.MtlBalJam.visitingPage.travelBlogSection.${section}.${subSection}`)
    return (
...
export default function TravelBlog() {
    const t = useTranslations('Events.2024.MtlBalJam.visitingPage.travelBlogSection')
    return (
