...
export default function NavTab({ tab, isOpen, setToggledTab, children, isMobile = false }: NavTabProps) {
    ...
    return (
        <li className={styles.parentTabs}>
            <button
                ...
                className={`${styles.tabButton} ${isOpen || isMobile ? styles.activeTab : ''}`}
            >
                {t(`${tab}.title`)}
                <Image src='/mbj-knife-black.png' alt='Toaster' width={18} height={18} />
            </button>
            <ul
                id={`menu-${tab}`}
